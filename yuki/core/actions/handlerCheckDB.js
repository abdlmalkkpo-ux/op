...
const { creatingThreadData, creatingUserData } = global.DB.database;
...
module.exports = async function (usersData, threadsData, event) {
  ...
  if (threadID && isGroup) {
    try {
      ...
      if (err.name != "DATA_ALREADY_EXISTS") {
        ...
        log.err("DATABASE", `Can't create group ${threadID}`, err);
      }
    }
  }
  ...
  if (senderID) {
    try {
      ...
      if (err.name != "DATA_ALREADY_EXISTS")
        log.err("DATABASE", `Can't create user ${senderID}`, err);
    }
  }
};