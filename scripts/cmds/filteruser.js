...
module.exports = {
	config: {
		...
		description: {
			...
			ar: "طرد أصنام الجروب و الحسابات الطائرة."
		},
		category: "box chat",
		guide: {
			vi: "   {pn} [<số tin nhắn> | die]",
			en: "   {pn} [<number of messages> | die]",
			ar: "   {pn} [عدد الرسائل | موتى]"
		}
	},
	...
	langs: {
		...
		ar: {
			needAdmin: "⚠️ | خلي البوت أدمن عشان الميزة",
			confirm: "⚠️ | تريد طرد من عدد رسائله أقل من %1 رسالة حقا ؟?\nرياكت كي أبدأ",
			kickByBlock: "✅ | تم طرد  %1 حساب ميت",
			kickByMsg: "✅ | تم طرد  %1 اشخاص لأن رسائلهم أقل من  %2 رسالة.",
			kickError: "❌ | لا يمكن طرد  %1 شخص:\n%2",
			noBlock: "✅ | كل من في المجموعة حسابه جيد",
			noMsg: "✅ | لا يوجد شخص رسائله أقل من  %1 رسالة في المجموعة"
		}
	},
	...
	onStart: async function ({ api, args, threadsData, message, event, commandName, getLang }) {
		...
		else if (args[0] == "die" || args[0] == "موتى") {
